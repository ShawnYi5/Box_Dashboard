...
def convertJSON2XML(root_element, json_doc):
    ...
    return parseString(
        xml_document.encode('ascii', 'xmlcharrefreplace').encode("utf-8").replace("\n", "").replace("&", ""))
    ...
    for attr in filter(lambda x: x not in dir(object), dir(cls)):
        ...
        if len(xml_value) > 0:
            if isinstance(attr_type, list) and attr_type[0] in [int, float, long, str]:
                cls_dict[attr] = [x.childNodes[0].nodeValue for x in xml_value[0].childNodes if
                                  x.nodeType == x.ELEMENT_NODE]

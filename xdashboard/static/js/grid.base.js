...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		var p = $.extend(true,{
			...
			ExpandColumn: null,
			tree_root_level : 0,
			prmNames: {page:"p",rows:"rows", sort: "sidx",order: "sord", search:"_search", nd:"nd", id:"id",oper:"oper",editoper:"edit",addoper:"add",deloper:"del"},
			forceFit : false,
			gridstate : "visible",
			cellEdit: false,
			cellsubmit: "remote",
		...
		sortArrayData = function() {
			...
			if(ts.p.treeGrid) {
				...
				rows.sort(function(a, b) {
					...
					//if (a.sortKey > b.sortKey) {return newDir;}
					//return 0;
					return newDir*a.sortKey.localeCompare(b.sortKey);
				});
				if(rows[0]){
		...
		$(ts).before(grid.hDiv).click(function(e) {
			...
			if (typeof(opts.URLFilter) != 'undefined') {
				ts.p.url = ts.p.url.replace(/\?.+/gi,'');
				ts.p.url = ts.p.url + '?a=list&'+opts.URLFilter;
			}

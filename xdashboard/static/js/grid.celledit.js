...
var pageinfo = function(){
	return{
		...
		_width:function(){return self.innerWidth||document.documentElement.clientWidth},
		_height:function(){return self.innerHeight||document.documentElement.clientHeight},
		height:function(){
...
$.jgrid.extend({
		...
		return this.each(function (){
			...
			if ($t.p.colModel[iCol].editable===true && ed===true && !cc.hasClass("not-editable-cell")) {
				...
				if ($.isFunction($t.p.beforeEditCell)) {
					tmp = $t.p.beforeEditCell($t.rows[iRow].id,nm,tmp,iRow,iCol);
				}
				...
				$t.p.savedRow.push({id:iRow,ic:iCol,name:nm,v:$(cc).html()});
				...
				$(cc).html('<div style="display:none" id="emDomModel"></div><div style="position:relative;z-index:10"></div>').find('> div:eq(1)').append(elc).attr("tabindex","0").attr("id","temptextarea");
				$($t).jqGrid("promptEX",{alpha:80,duration:100},$t);
				//$t.promptEX({alpha:80,duration:100});
	...
	promptEX: function(oi,et){
			var m = $('#emDomModel');
			m.css('position','absolute');
			m.css('top','0px');
			m.css('left','0px');
			m.css('background','#000');
			m.css('opacity',oi.alpha/100);
			//m.css('filter','alpha(opacity='+oi.alpha+')');
			

			m.css('height',pageinfo.height()+'px');
			m.css('width',pageinfo.width()+'px');
			//m.css('z-index','3000');
			//m.css('display','block');
			m.fadeIn(oi.duration);
			m.click(function(e){
				$(et).jqGrid("saveCell",et.p.savedRow[0].id,et.p.savedRow[0].ic);		
			});
	},
